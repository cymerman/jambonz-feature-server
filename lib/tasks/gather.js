...
class TaskGather extends SttTask {
    ...
    switch (this.vendor) {
      ...
      case 'speechmatics':
        this.bugname = `${this.bugname_prefix}speechmatics_transcribe`;
          this.addCustomEventListener(
              ep, SpeechmaticsTranscriptionEvents.EndOfUtterance, this._onSpeechmaticsEndOfUtterance.bind(this, cs, ep));
          this.addCustomEventListener(
    ...
    if (evt.is_final) {
      ...
      if (!emptyTranscript && sttLatency) {
        ...
        if (this.vendor === 'speechmatics' && !this.isContinuousAsr) {
            // Speechmatics wysyła najpierw final transcript, potem EndOfUtterance event
            // Więc musimy bufferować i czekać na EndOfUtterance
            if (!emptyTranscript) {
                const t = evt.alternatives[0].transcript;
                if (t) {
                    /* remove trailing punctuation */
                    if (/[,;:\.!\?]$/.test(t)) {
                        this.logger.debug('TaskGather:_onTranscription - removing trailing punctuation from speechmatics');
                        evt.alternatives[0].transcript = t.slice(0, -1);
                    }
                }
                this.logger.debug({evt}, 'Gather:_onTranscription - buffering speechmatics final transcript, waiting for EndOfUtterance');
                this._bufferedTranscripts.push(evt);
                this._clearTimer();
                return; // ⚠️ NIE resolvuj jeszcze - czekamy na EndOfUtterance!
            } else {
                // Empty transcript - może to oznaczać że użytkownik nic nie powiedział
                // Ale EndOfUtterance i tak przyjdzie, więc też bufferujemy
                this.logger.debug('Gather:_onTranscription - speechmatics empty final transcript, waiting for EndOfUtterance');
                return;
            }
        }
  ...
  async _onSpeechmaticsInfo(_cs, _ep, evt) {
    ...
    async _onSpeechmaticsEndOfUtterance(cs, ep, evt) {
        this.logger.debug({evt}, 'TaskGather:_onSpeechmaticsEndOfUtterance');

        if (this.bargein && this.minBargeinWordCount === 0) {
            this._killAudio(cs);
        }

        // Jeśli mamy zbuforowane transkrypty, zwróć je
        if (this._bufferedTranscripts.length > 0) {
            this.logger.info('TaskGather:_onSpeechmaticsEndOfUtterance - returning buffered transcripts');
            const consolidatedEvt = this.consolidateTranscripts(this._bufferedTranscripts, 1, this.language, this.vendor);
            this._bufferedTranscripts = [];
            this._resolve('speech', consolidatedEvt);
        }
    }
